...
// sections
import { CustomerTableRow, CustomerTableToolbar } from '../../../sections/@dashboard/tiffin/customer/list';
...
CustomersPage.getLayout = (page: React.ReactElement) => <DashboardLayout>{page}</DashboardLayout>;
...
export default function CustomersPage() {
  ...
  const handleEditRow = (id: number) => {
    push(PATH_DASHBOARD.tiffin.customerEdit(String(id)));
  };
    ...
    try {
      ...
      for (const line of dataLines) {
        try {
          ...
          const customerData = {
            name: cleanValues[1] || cleanValues[0],
            phone: cleanValues[2] || '',
          ...
          if (!customerData.name) {
            errorCount++;
            errors.push(`Line skipped: missing name`);
            continue;
          }
      ...
      if (successCount > 0) {
        enqueueSnackbar(`Imported ${successCount} customer(s) successfully`, { variant: 'success' });
        dispatch(getCustomers());
      }

      if (errorCount > 0) {
        const errorMsg = `${errorCount} customer(s) failed. ${errors.slice(0, 3).join(', ')}${
          errors.length > 3 ? '...' : ''
        }`;
  ...
  const handleExport = () => {
    ...
    const csvData = dataFiltered.map((customer) => [
      customer.id,
      `"${customer.name || ''}"`,
      `"${customer.phone || ''}"`,
      `"${customer.address || ''}"`,
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              onClick={() => push(PATH_DASHBOARD.tiffin.customerNew)}
            >
              New Customer
            </Button>
          }
  ...
  if (filterName) {
    inputData = inputData.filter(
      (customer) =>
        ...
        customer.phone?.toLowerCase().indexOf(filterName.toLowerCase()) !== -1 ||
        customer.address?.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
