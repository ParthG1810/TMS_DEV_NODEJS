...
export function createMealPlan(mealPlanData: IMealPlanFormValues) {
  return async (dispatch: Dispatch) => {
    ...
    try {
      ...
      console.error(error);
      dispatch(slice.actions.hasError(error));
      return { success: false, error: error.error || error.message || 'Failed to create meal plan' };
    }
  };
}
...
export function updateMealPlan(id: number, mealPlanData: Partial<IMealPlanFormValues>) {
  return async (dispatch: Dispatch) => {
    ...
    try {
      ...
      console.error(error);
      dispatch(slice.actions.hasError(error));
      return { success: false, error: error.error || error.message || 'Failed to update meal plan' };
    }
  };
}
...
export function deleteMealPlan(id: number) {
  return async (dispatch: Dispatch) => {
    ...
    try {
      ...
      console.error(error);
      dispatch(slice.actions.hasError(error));
      return { success: false, error: error.error || error.message || 'Failed to delete meal plan' };
    }
  };
}
