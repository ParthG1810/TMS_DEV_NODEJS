...
export function registerSetupIPC(): void {
  ...
  ipcMain.handle('setup:testConnection', async (_event, dbConfig: {
    host: string;
    port: number;
    name: string;
    user: string;
    ...
    log.info(`Setup: Testing connection to ${dbConfig.host}:${dbConfig.port}/${dbConfig.name}`);
  ...
  // Save config
  ipcMain.handle('setup:saveConfig', async (_event, config: Partial<AppConfig>) => {
