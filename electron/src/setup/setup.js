...
function populateForm(cfg) {
  ...
  if (cfg.app) {
    document.getElementById("minimizeToTray").checked =
      cfg.app.minimizeToTray !== false;
    document.getElementById("checkUpdates").checked =
      cfg.app.checkUpdates !== false;
  }
}
...
async function saveAndShowSummary() {
  ...
  try {
    ...
    document.getElementById(
      "summaryDb"
    ).textContent = `${formData.database.user}@${formData.database.host}:${formData.database.port}/${formData.database.name}`;

    // Show OAuth status
    const oauthConfigured =
      formData.google.clientId && formData.google.clientSecret;
    document.getElementById("summaryOAuth").textContent = oauthConfigured
      ? "Configured"
      : "Not configured";
  } catch (error) {
